...
public class AdvancedSettingActivity extends Activity implements IRegisterIOTCListener {
	...
	private LinearLayout pnlSyncToCloud;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		pnlSyncToCloud = (LinearLayout) findViewById(R.id.panelSyncToCloud);
		...
		mSyncSwitch = (Switch) findViewById(R.id.switchSyncToCloud);
		settings = this.getSharedPreferences("Login", 0);
		mIsLogin = settings.getBoolean("IsLogin", true);
		...
		mSyncSwitch.setOnCheckedChangeListener(switchSyncListener);
...
//			pnlVideoQuality.setVisibility(View.VISIBLE);
		...
		if (mCamera != null && mCamera.getSyncToCloudSupport(0) && mIsLogin) {
			pnlSyncToCloud.setVisibility(View.VISIBLE);
		} else {
			pnlSyncToCloud.setVisibility(View.GONE);
		}
	...
	private void initVideoSetting() {
		...
		ArrayAdapter<CharSequence> adapterVideoQuality = ArrayAdapter.createFromResource(this, R.array.video_quality,
		...
		ArrayAdapter<CharSequence> adapterEnvironment = ArrayAdapter.createFromResource(this, R.array.environment_mode,
				android.R.layout.simple_spinner_item);
		...
		if (mCamera != null) {
			...
			mCamera.sendIOCtrl(Camera.DEFAULT_AV_CHANNEL, AVIOCTRLDEFs.IOTYPE_USER_IPCAM_GETSTREAMCTRL_REQ,
					AVIOCTRLDEFs.SMsgAVIoctrlGetStreamCtrlReq.parseContent(mDevice.ChannelIndex));
			mCamera.sendIOCtrl(Camera.DEFAULT_AV_CHANNEL, AVIOCTRLDEFs.IOTYPE_USER_IPCAM_GET_VIDEOMODE_REQ,
					AVIOCTRLDEFs.SMsgAVIoctrlGetVideoModeReq.parseContent(mDevice.ChannelIndex));
			mCamera.sendIOCtrl(Camera.DEFAULT_AV_CHANNEL, AVIOCTRLDEFs.IOTYPE_USER_IPCAM_GET_ENVIRONMENT_REQ,
					AVIOCTRLDEFs.SMsgAVIoctrlGetEnvironmentReq.parseContent(mDevice.ChannelIndex));
		}
	...
	private void initDeviceInfo() {
		...
		if (mCamera != null) {
			mSyncSwitch.setChecked(manager.isDeviceSyncToCloud(mCamera.getUID()));
	...
	private void quit(boolean isPressOK) {
		...
		if (isPressOK) {
			...
			DatabaseManager manager = new DatabaseManager(this);
			manager.updateDeviceInfoByDBID(mDevice.DBID, mDevice.UID, mDevice.NickName, "", "", "admin", mDevice.View_Password,
			...
			if (mCamera != null) {
				...
				if (mVideoQuality != -1 && mVideoQuality != videoQuality) {
					mCamera.sendIOCtrl(Camera.DEFAULT_AV_CHANNEL, AVIOCTRLDEFs.IOTYPE_USER_IPCAM_SETSTREAMCTRL_REQ,
							AVIOCTRLDEFs.SMsgAVIoctrlSetStreamCtrlReq.parseContent(mDevice.ChannelIndex, (byte) videoQuality));
				}
				...
				if (mVideoFlip != -1 && mVideoFlip != videoFlip) {
					mCamera.sendIOCtrl(Camera.DEFAULT_AV_CHANNEL, AVIOCTRLDEFs.IOTYPE_USER_IPCAM_SET_VIDEOMODE_REQ,
							AVIOCTRLDEFs.SMsgAVIoctrlSetVideoModeReq.parseContent(mDevice.ChannelIndex, (byte) videoFlip));
				}
				...
				if (mEnvMode != -1 && mEnvMode != envMode) {
					mCamera.sendIOCtrl(Camera.DEFAULT_AV_CHANNEL, AVIOCTRLDEFs.IOTYPE_USER_IPCAM_SET_ENVIRONMENT_REQ,
							AVIOCTRLDEFs.SMsgAVIoctrlSetEnvironmentReq.parseContent(mDevice.ChannelIndex, (byte) envMode));
				}
				...
				if (mMotionDetection != -1 && mMotionDetection != motionDetection) {
					...
					mCamera.sendIOCtrl(Camera.DEFAULT_AV_CHANNEL, AVIOCTRLDEFs.IOTYPE_USER_IPCAM_SETMOTIONDETECT_REQ,
							AVIOCTRLDEFs.SMsgAVIoctrlSetMotionDetectReq.parseContent(mDevice.ChannelIndex, md));
		...
		boolean isSettingsChanged = isPressOK
				&& ((mVideoQuality != -1 && videoQuality != mVideoQuality) || (mVideoFlip != -1 && videoFlip != mVideoFlip)
						|| (mEnvMode != -1 && envMode != mEnvMode) || (mRecordType != -1 && recType != mRecordType)
						|| (mMotionDetection != -1 && motionDetection != mMotionDetection) || (mPostition != -1 && postition != mPostition));
...
//        Arrays.sort(timeZoneNameList);
	...
	private View.OnClickListener btnManageWiFiNetworksOnClickListener = new View.OnClickListener() {
		...
		public void onClick(View v) {
			...
			/* if wifi list is empty, all control but cancel button must be disabled */
			...
			checkBox.setPadding(checkBox.getPaddingLeft() + (int) (5.0f * scale), checkBox.getPaddingTop(), checkBox.getPaddingRight(),
			...
			btnOK.setOnClickListener(new View.OnClickListener() {
				...
				public void onClick(View v) {
					...
					/*if (pwd.length() == 0) { Toast.makeText(AdvancedSettingActivity.this,
					 * getText(R.string.tips_all_field_can_not_empty).toString(),
					...
					if (mCamera != null && wifi != null) {
						...
						mCamera.sendIOCtrl(Camera.DEFAULT_AV_CHANNEL, AVIOCTRLDEFs.IOTYPE_USER_IPCAM_SETWIFI_REQ,
								AVIOCTRLDEFs.SMsgAVIoctrlSetWifiReq.parseContent(wifi.ssid, pwd.getBytes(), wifi.mode, wifi.enctype));
	...
	private View.OnClickListener btnFormatSDCardListener = new View.OnClickListener() {
		...
		public void onClick(View v) {
			...
			new AlertDialog.Builder(AdvancedSettingActivity.this).setIcon(android.R.drawable.ic_dialog_alert)
					.setTitle(getText(R.string.tips_warning)).setMessage(getText(R.string.tips_format_sdcard_confirm))
			...
			btnOK.setOnClickListener(new View.OnClickListener() {
				...
				public void onClick(View v) {
					...
					if (oldPwd.length() == 0 || newPwd.length() == 0 || confirmPwd.length() == 0) {
						Toast.makeText(AdvancedSettingActivity.this, getText(R.string.tips_all_field_can_not_empty).toString(), Toast.LENGTH_SHORT)
								.show();
						return;
					}

					if (!oldPwd.equalsIgnoreCase(mDevice.View_Password)) {
						Toast.makeText(AdvancedSettingActivity.this, getText(R.string.tips_old_password_is_wrong).toString(), Toast.LENGTH_SHORT)
								.show();
						return;
					}

					if (!newPwd.equalsIgnoreCase(confirmPwd)) {
						Toast.makeText(AdvancedSettingActivity.this, getText(R.string.tips_new_passwords_do_not_match).toString(), Toast.LENGTH_SHORT)
								.show();
						return;
					}
	...
	private OnCheckedChangeListener switchSyncListener = new OnCheckedChangeListener() {

		@Override
		public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
			// TODO Auto-generated method sub
			DatabaseManager manager = new DatabaseManager(AdvancedSettingActivity.this);
			manager.updateDeviceIsSyncByUID(mCamera.getUID(), isChecked);
		}
	};
	...
	private Handler handler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			case AVIOCTRLDEFs.IOTYPE_USER_IPCAM_SETPASSWORD_RESP:
				...
				if (data[0] == 0x00)
					Toast.makeText(AdvancedSettingActivity.this, getText(R.string.tips_modify_security_code_ok).toString(), Toast.LENGTH_SHORT)
							.show();
	...
	class ThreadCheck extends Thread {
		...
		public void run() {
			do {
				// for Marco嚝迎噥嚝恬�嚙�噫50嚝穿翮嚝再嚝堆�嚝莎�嚝二嚝佗噪
