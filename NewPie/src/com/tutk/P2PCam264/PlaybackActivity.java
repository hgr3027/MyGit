...
public class PlaybackActivity extends SherlockActivity implements IRegisterIOTCListener {
	...
	private void setupViewInLandscapeLayout_wait_Mediacodec() {
		...
		case AVFrame.MEDIA_CODEC_VIDEO_H264:
			setContentView(R.layout.playback_landscape_h264);
			break;

		case AVFrame.MEDIA_CODEC_VIDEO_MPEG4:
			setContentView(R.layout.playback_landscape_mpeg4);
			break;
	...
	private void setupViewInPortraitLayout_wait_Mediacodec() {
		...
		case AVFrame.MEDIA_CODEC_VIDEO_H264:
			setContentView(R.layout.playback_portrait_h264);
			break;

		case AVFrame.MEDIA_CODEC_VIDEO_MPEG4:
			setContentView(R.layout.playback_portrait_mpeg4);
			break;
	...
	public void receiveFrameData(final Camera camera, int sessionChannel, Bitmap bmp) {
		...
		if(wait_receive) {
			if (mCamera != null) {
				...
				mCamera.stopSpeaking(mPlaybackChannel);
				mCamera.stopListening(mPlaybackChannel);
				mCamera.stopShow(mPlaybackChannel);
			}
			...
			if (rotation == Surface.ROTATION_0 || rotation == Surface.ROTATION_180){
				runOnUiThread(new Runnable() {
					...
					public void run() {
						...
						mCamera.startShow(mPlaybackChannel,true,false);
					}
				});
			}
			else{
					...
					if (mMediaState == MEDIA_STATE_OPENING) {
						if (0 <= result && result <= 31) {
							...
							if (mCamera != null) {
								...
								mCamera.startShow(mPlaybackChannel,false,false);
								mCamera.startListening(mPlaybackChannel,mIsRecording);
//								monitor.enableDither(mCamera.mEnableDither);
//								monitor.attachCamera(mCamera, mPlaybackChannel);
							}
